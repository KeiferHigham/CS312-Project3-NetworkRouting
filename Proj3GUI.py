...
#			self.view.repaint()
	...
	def computeClicked(self):
		...
		if self.useUnsorted.isChecked():
			...
			doHeap = True
		if doArray:
			array_time = self.solver.computeShortestPaths( int(self.sourceNode.text())-1,False )
			array_path = self.solver.getShortestPath( int(self.targetNode.text())-1 )
			dist = array_path['cost']
		if doHeap:
			heap_time = self.solver.computeShortestPaths( int(self.sourceNode.text())-1,True )
